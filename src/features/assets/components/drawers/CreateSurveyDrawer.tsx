        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 font-['Manrope']">
            ...
            <section className="space-y-4">
              <h3 className="text-13 font-semibold uppercase tracking-wide text-muted-foreground">Survey Details</h3>
              ...
              <div>
                <Label className="text-15 mb-1.5 block">Scope <span className="text-destructive">*</span></Label>
                <Textarea {...register('scope')} placeholder="Define which assets are in scope (categories, locations, value threshold…)"
                  rows={2} className="text-15" />
                {errors.scope && <p className="text-13 text-destructive mt-1">{errors.scope.message}</p>}
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-15 mb-1.5 block">Survey Type <span className="text-destructive">*</span></Label>
                  ...
                  {errors.surveyType && <p className="text-13 text-destructive mt-1">{errors.surveyType.message}</p>}
                </div>
                <div>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-15 mb-1.5 block">Team Lead <span className="text-destructive">*</span></Label>
                  <Select onValueChange={(v) => setValue('surveyTeamLead', v, { shouldValidate: true })}>
                    <SelectTrigger className="h-10 text-15"><SelectValue placeholder="Select team lead" /></SelectTrigger>
                    <SelectContent>
                  ...
                  {errors.surveyTeamLead && <p className="text-13 text-destructive mt-1">{errors.surveyTeamLead.message}</p>}
                </div>
                <div>
              ...
              <div>
                <Label className="text-15 mb-1.5 block">Target Locations <span className="text-destructive">*</span></Label>
                <Select onValueChange={(v) => setValue('targetLocations', v, { shouldValidate: true })}>
                  <SelectTrigger className="h-10 text-15"><SelectValue placeholder="Select primary location" /></SelectTrigger>
                  <SelectContent>
                    {LOCATIONS.map((l) => (
                ...
                {errors.targetLocations && <p className="text-13 text-destructive mt-1">{errors.targetLocations.message}</p>}
              </div>
            </section>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-15 mb-1.5 block">Start Date <span className="text-destructive">*</span></Label>
                  <Input type="date" {...register('plannedStartDate')} className="h-10 text-15" />
                  {errors.plannedStartDate && <p className="text-13 text-destructive mt-1">{errors.plannedStartDate.message}</p>}
                </div>
                <div>
                  <Label className="text-15 mb-1.5 block">End Date</Label>
                  <Input type="date" {...register('plannedEndDate')} className="h-10 text-15" />
                </div>
              </div>
            </section>
