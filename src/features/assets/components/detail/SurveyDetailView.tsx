...
function SectionHead({ children }: { children: React.ReactNode }) {
  return <p className="text-13 font-semibold uppercase tracking-wide text-muted-foreground mb-3">{children}</p>
}
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {stats.map(t => (
          <div key={t.label} className="p-4 rounded-[6px] bg-muted/40 border">
            <p className="text-13 text-muted-foreground">{t.label}</p>
      ...
      <div className="p-4 rounded-[6px] bg-muted/40 border space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-13 text-muted-foreground">Accuracy Rate</span>
          <span className="text-13 font-semibold font-['Manrope']">{rate}%</span>
        </div>
      ...
      {(survey.scope || survey.description) && (
        <div className="p-4 rounded-[6px] bg-muted/40 border space-y-1">
          ...
          {survey.scope       && <p className="text-15 font-['Manrope']">{survey.scope}</p>}
          {survey.description && <p className="text-13 text-muted-foreground font-['Manrope']">{survey.description}</p>}
        </div>
      )}

      {survey.linkedDisposalId && (
        <div className="p-3 rounded-[6px] bg-blue-500/10 border border-blue-500/20 flex items-center gap-2 text-13 text-blue-700 dark:text-blue-300 font-['Manrope']">
      ...
      {survey.assetsLocked && (
        <div className="p-3 rounded-[6px] bg-amber-500/10 border border-amber-500/20 flex items-center gap-2 text-13 text-amber-700 dark:text-amber-300 font-['Manrope']">
    ...
    <div className="p-4 rounded-[6px] bg-muted/40 border overflow-x-auto">
      <table className="w-full text-15">
        ...
        <tfoot className="border-t">
          <tr>
            <td colSpan={8} className="pt-2 text-13 text-muted-foreground font-semibold">
  ...
  if (!discrepancyAssets.length) {
    return (
      <div className="flex flex-col items-center gap-2 py-16 text-muted-foreground">
        ...
        <p className="text-15">No discrepancies found</p>
      </div>
    )
  }
      ...
      {discrepancyAssets.map(a => (
        <div key={a.id} className="p-4 rounded-[6px] border mb-3">
          <div className="flex items-start justify-between gap-3 mb-2">
            <div>
              <p className="text-15 font-bold font-['Manrope']">{a.name}</p>
              <p className="text-13 text-muted-foreground font-['Manrope']">{a.assetId}</p>
            </div>
            {a.discrepancy && (
              <Badge className={getDiscrepancyColor(a.discrepancy)}>{getDiscrepancyLabel(a.discrepancy)}</Badge>
            )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-[6px] bg-muted/40 border flex items-center gap-3">
        ...
        <span className="text-15 font-['Manrope']">{survey.reportGenerated ? 'Survey Report Generated' : 'Report Pending'}</span>
      ...
      <div className="p-4 rounded-[6px] bg-muted/40 border">
        ...
        {survey.attachments.length === 0
          ? <p className="text-13 text-muted-foreground font-['Manrope']">No attachments uploaded.</p>
          : (
            <div className="space-y-2">
              {survey.attachments.map((file, i) => (
                <div key={i} className="flex items-center gap-3 py-1.5 border-b last:border-0">
                  <FileText className="w-4 h-4 text-muted-foreground shrink-0" />
                  <span className="text-15 font-['Manrope'] flex-1">{file}</span>
